...
Timer TimerActive; // global timer
bool WritingMode = false;


void PromtTimerAndStartCalibration() {
	TimerActive.getCurrentTime();
	TimerActive.PromtUserForTimeAndwaitTillit();
}
...
void Kinect::run()
{
	...
	string savePath = "G://Kinect_data//Calibration";
	//create folder
	if (mkdir(savePath.c_str()) != 0)
    ...
    while( true ){
		...
		//save data if the Calibration was launched
		if (WritingMode && TimerActive.IntervalTimePassed()) {
			saveData(Frame, savePath);
